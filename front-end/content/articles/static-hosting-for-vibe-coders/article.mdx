...
**Explicit non-goals:** backends, databases, per-app auth, custom domains per app, and edge response header policies (apps set `Cache-Control` in their build output). This stays a **static** lane on purpose.
...
- Deploy workflows invalidate `/{app}/{branch}/*` and `/{app}/404.html` after upload.

Apps should set sensible `Cache-Control` in the build: short TTL for HTML, long immutable TTL for hashed assets.
