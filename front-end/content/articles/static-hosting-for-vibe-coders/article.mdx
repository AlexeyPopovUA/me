...
An app repo does **not** copy CDK stacks, bucket names, or distribution IDs. It adds a short `deploy.yml` that calls the platform’s reusable `deploy-app.yml` via GitHub Actions `workflow_call`, passes its `app-slug` and build settings, and inherits the shared OIDC role. The platform workflow builds inside the caller’s checkout, syncs to `s3://…/{app}/{branch}/`, invalidates CloudFront, and — on pull requests — comments the preview URL.
...
The platform exposes three workflows meant for **external** app repositories:

| Workflow | Trigger in app repo | What it does |
|----------|---------------------|--------------|
| `deploy-app.yml` | `push` to `main`, `pull_request` | Build, sanitize branch name, `s3 sync`, invalidate CloudFront, comment preview URL on PRs |
| `cleanup-branch.yml` | `delete` (remote branch removed) | Remove `/{app}/{branch}/` from S3 and invalidate that prefix |
| `invalidate-app.yml` | Manual / optional | Cache bust without redeploying |

App repos stay thin. Typical `deploy.yml` is under 25 lines — only permissions, triggers, `app-slug`, build command, output directory, and `base-domain`:
...
`secrets: inherit` forwards the shared `AWS_AUTH_ROLE` GitHub variable for OIDC. Bucket and distribution IDs are **not** secrets in app repos — `deploy-app.yml` reads `/static-hosting/bucket-name` and `/static-hosting/distribution-id` from SSM at runtime (see [Infra IDs in SSM](#infra-ids-in-ssm)).

Inside each run, the reusable workflow:

1. Checks out the **calling app repo** (your code).
2. Checks out the **platform repo** into `.platform` (validation scripts only).
3. Runs `pnpm install` + your `build-command` in the app repo.
4. Sanitizes the branch via `validate-names.ts` (e.g. `test/feature-branch-hosting` → `test-feature-branch-hosting`).
5. Assumes the OIDC role, syncs `dist/` to the correct S3 prefix, invalidates CloudFront paths, and posts or updates a PR comment with `https://{app}--{branch}.dev.{base-domain}`.

Optional `cleanup.yml` mirrors the same `app-slug` and calls `cleanup-branch.yml` when GitHub fires `delete` for a branch — so preview folders do not linger after merge or abandoned experiments.
...
`secrets: inherit` passes the shared `AWS_AUTH_ROLE` for OIDC — not bucket or distribution IDs. The reusable workflow [looks those up from SSM](#infra-ids-in-ssm) at runtime.
